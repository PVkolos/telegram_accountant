...
async def write(date, name, razd, kat, summ, prim, sheet, SAMPLE_SPREADSHEET_ID):
    ...
    try:
        ...
        pass
    del service, array, n, range_


async def write_c(sheet, where, what, n, SAMPLE_SPREADSHEET_ID):
    service = build('sheets', 'v4', credentials=credentials).spreadsheets().values()
    array = {'values': [[what]]}
    range_ = f'{sheet}!{where}{n + 1}'
    service.update(spreadsheetId=SAMPLE_SPREADSHEET_ID,
                   range=range_,
                   valueInputOption='USER_ENTERED',
                   body=array).execute()
...
async def create_table(SAMPLE_SPREADSHEET_ID, arr=None):
    ...
    if not arr:
        array = {'values': [['Раздел', 'Категория', 'Ключевые слова (через запятую и пробел)'],
                            ...
                            ['Потребность', 'Связь'],
                            ['Потребность', 'Одежда'],
                            ['Расходы по обязательствам', 'Кредит'],
                            ['Расходы по обязательствам', 'ЖКХ'],
    ...
    service.update(spreadsheetId=SAMPLE_SPREADSHEET_ID, range=range_, valueInputOption='USER_ENTERED',
                   body=array).execute()
    del service, range_, array, body1, body2, req1, req2
    await color(SAMPLE_SPREADSHEET_ID, 'Категории и разделы', 0.33, 0.65, 0.73, 0, 1, 0, 3)
    await color(SAMPLE_SPREADSHEET_ID, 'Записи бота', 0.33, 0.65, 0.73, 0, 1, 0, 6)
...
async def color(spreadsheet_id, name_sheet, r, g, b, startR, endR, startC, endC):
    ...
    del service, body, id_sheet, spreadsheet, sheet, sheetList
...
def deletsheet(spreadsheet_id):
    ...
    body = {'requests': body}
    service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()
    del service, body, spreadsheet, sheetList, id_sheet, sheet
...
async def analytic(spreadsheet_id):
    ...
    aftors = dict()
    for el in sheet[1:]:
        if el[1] not in aftors:
            aftors[el[1]] = {'Доходы': 0, 'Расходы': 0}
        if el[2] == 'Доходы':
            aftors[el[1]]['Доходы'] += int(el[4])
        else:
            aftors[el[1]]['Расходы'] += int(el[4])
    body_values.append([])
    body_values.append([])
    for el in aftors:
        body_values.append([f'Доходы {el}', aftors[el]['Доходы'], ' ', f'Расходы {el}', aftors[el]['Расходы']])
...
if __name__ == '__main__':
    # service = build('sheets', 'v4', credentials=credentials).spreadsheets().values()
    # array = {'values': [['string']]}
    # range_ = f'Записи бота!A332'
    # service.update(spreadsheetId='1VJ_CZwhegeRon8evbjTBKPY-zEC8cssdEK6BY_mYSGQ',
    #                range=range_,
    #                valueInputOption='USER_ENTERED',
    #                body=array).execute()
    loop = asyncio.get_event_loop()
    loop.run_until_complete(read('Категории и разделы', '1VJ_CZwhegeRon8evbjTBKPY-zEC8cssdEK6BY_mYSGQ'))
    loop.close()

...
async def use_help(message):
    await send(message.from_user.id,
               ...
               'но их <u>можно добавлять (вручную в гугл таблице на листе "Категории и разделы")</u>. Также, можно <u>добавлять и категории, и ключевые слова</u>. Категории и '
               'ключевые слова можно добавлять <u>прямиком из бота (для этого вам достаточно сделать новую запись с незнакомым словом, бот его определит сам)</u>.\n\n'
               ...
               'Вы можете задать две таблицы для записей. Одна, например, для семейного бюджета, а вторая - для личного. Воспользуйтесь командой /add_tables\n'
               'В случае неполадок обращайтесь к администратору командой /support\n\n'
